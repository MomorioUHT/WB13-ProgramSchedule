...
def purge_old_records(cutoff_raw: str, sheet_names: list[str], dry_run: bool = False) -> int:
    ...
    log.info(
        ...
        grand_removed, "would be removed" if dry_run else "removed", total - len(failed),
    )
    if failed:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Used with --purge: show how many rows would be removed without changing the sheet",
    )
    return parser
